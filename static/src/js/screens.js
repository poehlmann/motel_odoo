...
var ScaleScreenWidget = ScreenWidget.extend({
    ...
    get_computed_price_string: function(){
        var result_price  = this.format_currency(this.get_product_price() * this.weight);
        console.log("resul_price",result_price);
        return result_price;
    },
    close: function(){
        this._super();
...
var OrderWidget = PosBaseWidget.extend({
    ...
    init: function(parent, options) {
        ...
        this.line_click_handler = function(event){
            ...
            console.log("click mesa");
        };

        console.log("tiene ordenes?",this.pos.get_order());
    ...
    bind_order_events: function() {
        ...
        console.log("bind_order_events - order",order);
        ...
        if (this.pos.get_order().orderlines.length>0) {
            ...
            console.log("agregando producto habitacion si no existe", this.pos.db.get_product_by_category(0));
            ...
            for (var productA in product_by_category) {
                ...
                if (product_by_category[productA].display_name == "Habitacion") {
...
var ProductScreenWidget = ScreenWidget.extend({
    ...
    start: function(){
        ...
        this.product_list_widget = new ProductListWidget(this,{
            click_product_action: function(product){ self.click_product(product); console.log("click!! product_list_widget");},
    ...
    click_product: function(product) {
       if(product.to_weight && this.pos.config.iface_electronic_scale){
           console.log("this.pos.config.iface_electronic_scale",this.pos.config.iface_electronic_scale);
           console.log("product.to_weight",product.to_weight);
           this.gui.show_screen('scale',{product: product});
       }else{
           console.log("Solicito una",product.display_name);
           console.log("get_product_by_category",this.pos.db.get_product_by_category(0));
...
var PaymentScreenWidget = ScreenWidget.extend({
	...
	if (! open_paymentline) {
            var did_pay = this.pos.get_order().add_paymentline( this.pos.cashregisters[0]);
            console.log("done_pay",did_pay);
            this.render_paymentlines();
        }
    ...
    click_paymentline: function(cid){
        ...
        for ( var i = 0; i < lines.length; i++ ) {
            if (lines[i].cid === cid) {
                var lines_products_pay = this.pos.get_order().select_paymentline(lines[i]);
                console.log("lines_products_pay",lines_products_pay);
                this.reset_input();
                this.render_paymentlines();
                return;
            }
...
/*
    addExtraHours : function (){
        ...
        var order = this.pos.get_order();
        var today = new Date();
        var room = order.creation_date;
        var cant = 0;
        var diffMs = (today - room); // milliseconds between now and the reserve of the room
        var diffHrs = Math.round((diffMs % 86400000) / 3600000); // hours
        var diffMins = Math.round(((diffMs % 86400000) % 3600000) / 60000); // minutes

        if(diffHrs > 1){
            cant = diffHrs;
            console.log("cant-H",cant);
        }

        if(diffMins>15){
            cant += 1;
            console.log("cant-M",cant);
        }

        for(var i=1 ; i<=cant ; i++){
            this.add_extra_hour(this.pos);
        }

    },
    add_extra_hour : function(pos){
        console.log("adicionando hora adicional");
        var product_by_category = pos.db.get_product_by_category(0);
        for (var productA in product_by_category) {
            console.log("buscando producto", product_by_category[productA]);
            if (product_by_category[productA].display_name == "Hora Adicional") {
                console.log("encontro!");
...
*/
    render_paymentlines: function() {
        var self  = this;
        var order = this.pos.get_order();
        console.log("render_paymentlines-order",order);
        if (!order) {
            return;
        }
        //this.addExtraHours();
        ...
        console.log("render_paymentlines-lines",lines);
